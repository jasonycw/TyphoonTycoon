	...
	AttackTower.prototype.tick = function(dt) { // override
		if (this.coolDownCounter <= 0) {
			...
			if (this.game.isBuilt('University')) {
				_range += Config.University.attackTowerRangeIncrease*this.game.numberOfBuilding('University');
				_attackDamage += Config.University.attackTowerAttackIncrease*this.game.numberOfBuilding('University');
			}
			if (this.game.isBuilt('ResearchCenter')) {
				_range += Config.ResearchCenter.attackTowerRangeIncrease*this.game.numberOfBuilding('ResearchCenter');
				_attackDamage += Config.ResearchCenter.attackTowerAttackIncrease*this.game.numberOfBuilding('ResearchCenter');
			}
			...
			if (nearestEnemy) {
				if (nearestEnemy.distance <= _range && this.game.getAvailablePower() > 0) {
